...
; void _xmod(limb_t* a, limb_t* b, int32_t da, int32_t db);  // a = (a mod b)
...
__xmod:
    ...
    ; cdecl parameters: &a, &b, d(a), d(b)
    mov     eax, [ebp+8]            ; &a
    mov     edx, [ebp+12]           ; &b
    mov     ebx, [ebp+16]           ; d(a)
    mov     ecx, [ebp+20]           ; d(b)
    mov     dword [Zdivq0], 1       ; set &q = 1 for apz_xdiv
    ...
    mov     dword [eax+ebx*4], 0    ; Extra zero for a
...
.Ldivp:
    ...
    dec     ebx
    jg      .Ldivp
    sub     dword [edi], ebp        ; Subtract final digit
...
.Ldivr:
    ...
    ; no quotient digit for xmod
.Ldivt:
    sub     dword [Zdivai], 4       ; &a_{i-1}
    dec     dword [Zdivii]          ; i--
